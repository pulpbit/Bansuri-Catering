...
import { MENU_CATALOG, SECTION_LABELS } from '../data/menu-catalog.js';
import { createElement, formatPackageLabel } from '../utils/helpers.js';
import { getSectionsForPackage, suggestMenuForLead, summarizeSelections } from '../utils/menu-engine.js';
...
function renderMenuSection(sectionKey, selectedValue = '') {
  const wrap = createElement('div', 'menu-section-item');
  const label = createElement('label', '', SECTION_LABELS[sectionKey] || sectionKey);
  const select = createElement('select');
  select.dataset.menuSection = sectionKey;
  select.innerHTML = '<option value="">Select an option</option>';

  (MENU_CATALOG[sectionKey] || []).forEach((item) => {
    const option = createElement('option', '', item);
    option.value = item;
    option.selected = selectedValue === item;
...
export function renderStep(contentRoot, step, state) {
  ...
  if (step === 3) {
    section.append(createElement('h3', '', 'Suggested menu + customize sections'));
    ...
    if (!selectedPackage) {
      ...
      const suggestionCard = createElement('div', 'suggestion-card');
      suggestionCard.innerHTML = `
        <p><strong>Auto suggestion:</strong> ${suggestion.label}</p>
        <p class="suggestion-note">Based on package, event type, and guest count.</p>
      `;
      const applyBtn = createElement('button', 'btn btn--secondary', 'Use this suggestion');
      applyBtn.type = 'button';
      applyBtn.dataset.applySuggestion = 'true';
      suggestionCard.append(applyBtn);

      menuWrap.append(createElement('p', '', `Package selected: ${formatPackageLabel(selectedPackage)}`));
      menuWrap.append(suggestionCard);

      const sectionsGrid = createElement('div', 'menu-section-grid');
      getSectionsForPackage(state.packageId).forEach((sectionKey) => {
        sectionsGrid.append(renderMenuSection(sectionKey, state.menuSelections?.[sectionKey]));
      });

      menuWrap.append(sectionsGrid);
  ...
  if (step === 4) {
    ...
    const rows = [
      ...
      ['Suggestion', state.menu || '-'],
      ['Menu Sections', summarizeSelections(state.menuSelections) || '-']
    ];
